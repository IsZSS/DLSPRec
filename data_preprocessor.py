...
def generate_POI_sequences(input_data, visit_sequence_dict):  # VenueId代表poi_id
...
def generate_category_sequences(input_data, visit_sequence_dict):
    ...
    for user in visit_sequence_dict:
        ...
        for seq in visit_sequence_dict[user]:
            single_cat_sequence = []
            for visit in seq:
                # 用的是大类 L1_Category，一共 10 个类别
                # 如果用小类 Category，大约 390 个类别
    ...
    return reIndexed_cat_sequences, cat_reIndex_mapping  # cat_reIndex_mapping 可以看看这两个文件的内容
...
def generate_day_sequences(input_data, visit_sequence_dict):
    ...
    input_data["is_weekend"] = pd.to_datetime(
        input_data['Local_Time_True']).dt.dayofweek > 4

    # input_data["dayofweek"] = pd.to_datetime(input_data['Local_Time_True']).dt.dayofweek
    ...
    for user in visit_sequence_dict:
        ...
        for seq in visit_sequence_dict[user]:
            ...
            for visit in seq:
                ...
                # single_day_sequence.append(input_data['dayofweek'][visit])
...
def filter_long_short_term_sequences(total_sequences_meta, min_short_term_len, pre_seq_window, min_long_term_count):
    ...
    avg_short_seq_len = user_short_nums / valid_input_count
    print(
        f"Filtered {valid_input_count} valid input long+short term sequences for {valid_user_count} users, avg short sequence length is {avg_short_seq_len}.")
...
def split_train_test(input_samples):  # 划分训练集
    ...
    if settings.cold_start:  # 重新产生稀疏数据集，将训练集（随机）5折，获取20%、40%、60% 80% 的数据
        tmp_training_samples = input_samples[:train_valid_boundary]
        M = len(tmp_training_samples)
        cold_start_boundary = int(settings.sparsity * M)  # 获得前  的训练数据集
        all_training_samples = tmp_training_samples[:cold_start_boundary]
    else:
