...
class TransRec(nn.Module):
    ...
    def forward(self, sample):
        ...
        if settings.enable_catpre_embedding:
            ...
            # 加入时间注意力机制，将当前时间（hour+day）作为查询健  
            ...
            #time_embedding = torch.cat((next_cat_embedding, next_time_embedding, next_day_embedding),dim=-1)  # 一维[embed_size*3]
            time_embedding = torch.cat((next_time_embedding, next_day_embedding),dim=-1)  # 一维[embed_size*2]
            time_embedding = time_embedding.unsqueeze(0)

            # 下一个poi访问 类别偏好的嵌入作为查询健
